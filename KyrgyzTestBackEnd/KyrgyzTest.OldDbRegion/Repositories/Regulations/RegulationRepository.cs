...
namespace KyrgyzTest.OldDbRegion.Repositories.Regulations;

public class RegulationRepository(LegacyDbRegionContext dbRegionContext) : IRegulationRegionRepository
{
    public async Task<RegulationDto?> GetByIdAsync(int id)
    {
        var response =  await dbRegionContext.Regulations.FirstOrDefaultAsync(e => e.Id == id);
    ...
    {
        var response =  await dbRegionContext.Regulations.ToListAsync();
